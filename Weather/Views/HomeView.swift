        ...
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                CurrentWeatherHeader(
                    prorated: bottomSheetTranslationProrated, hasDragged: hasDragged
                )
                .padding(.top, 75)
...
// Reads the store in its own body so a city change re-renders only this header,
// not the whole HomeView. Re-evaluating HomeView.body re-presents the bottom
// sheet, which makes the home screen jump.
private struct CurrentWeatherHeader: View {
    @Environment(WeatherStore.self) var store
    var prorated: CGFloat
    var hasDragged: Bool

    var body: some View {
        VStack(spacing: -10 * (1 - prorated)) {
            Text(store.currentForecast.city)
                .font(.largeTitle)
            ...
            VStack {
                ...
                Text(
                    TemperatureFormat.highLow(
                        high: store.currentForecast.high, low: store.currentForecast.low,
                        separator: "   ")
                )
    ...
    private var attributedString: AttributedString {
        let temperature = TemperatureFormat.degrees(store.currentForecast.temperature)
        let condition = store.currentForecast.weather.rawValue

        var string = AttributedString(temperature + (hasDragged ? "" : "\n") + condition)

        if let temp = string.range(of: temperature) {
            string[temp].font = .system(
                size: SheetMetrics.temperatureFontSize(prorated: prorated),
