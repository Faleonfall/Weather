...
struct ForecastView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 12) {
                        if selection == 0 {
                            ForEach(SampleForecasts.hourly) {
                                forecast in
                                ForecastCard(forecast: forecast, forecastPeriod: .hourly)
                            }
                            .transition(.offset(x: -430))
                        } else {
                            ForEach(SampleForecasts.weekly) {
                                forecast in
        ...
        .innerShadow(
            shape: RoundedRectangle(cornerRadius: 44), color: Color.bottomSheetBorderMiddle,
            lineWidth: 1, offsetX: 0, offsetY: 1, blur: 0, blendMode: .overlay,
